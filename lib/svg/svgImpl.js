...
class SvgImpl extends Component {
    render() {
        ...
        // 更新，忽略掉Group输入项
        const group = keys(CURRENT_PATHS).find(ele => CURRENT_PATHS[ele].hasOwnProperty(setting.name));
        if (!group) {
        ...
        paths.forEach((path, idx) => {
            shapes.push(React.createElement(Shape, { fill: setting["color" + idx] || setting.color, stroke: setting["stroke" + idx] || setting.stroke, strokeWidth: setting["strokeWidth" + idx] || setting.strokeWidth,
                d: path, key: idx }));
        });
        ...
        if (texts) {
            texts.forEach((text, idx) => {
                shapes.push(React.createElement(Text, { fill: setting["color" + (idxTotal + idx)] || setting.color, transform: new Transform(text.transform), font: text.font, children: text.text, key: idxTotal + idx }));
            });
        ...
        if (draws) {
            draws.forEach((draw, idx) => {
                ...
                shapes.push(React.createElement(Shape, _extends({}, props, {
